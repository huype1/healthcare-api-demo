...
export const getAppointments = asyncHandler(async (req, res) => {
    const { patient, doctor, facility, status, type, startDate, endDate, page = 1, limit = 10 } = req.query;
    const filter = {};
    if (patient)
...
export const createAppointment = asyncHandler(async (req, res) => {
    const { patient, doctor, facility, scheduledDate, duration = 30, type = 'consultation', notes } = req.body;
...
export const updateAppointment = asyncHandler(async (req, res) => {
    ...
    const updatedAppointment = await Appointment.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true }).populate('patient', 'firstName lastName email phone')
        .populate('doctor', 'fullName email role')
        .populate('facility', 'name address');
    res.status(200).json({
        success: true,
