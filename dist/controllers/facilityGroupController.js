...
export const listFacilityGroups = asyncHandler(async (_req, res) => {
    const groups = await FacilityGroup.find();
    res.json(groups);
});
export const getFacilityGroup = asyncHandler(async (req, res) => {
    const group = await FacilityGroup.findById(req.params.id);
    if (!group)
        return res.status(404).json({ error: 'FacilityGroup not found' });
    res.json(group);
});
export const createFacilityGroup = asyncHandler(async (req, res) => {
    const group = await FacilityGroup.create(req.body);
    res.status(201).json(group);
});
export const updateFacilityGroup = asyncHandler(async (req, res) => {
    const group = await FacilityGroup.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!group)
        return res.status(404).json({ error: 'FacilityGroup not found' });
    res.json(group);
});
export const deleteFacilityGroup = asyncHandler(async (req, res) => {
    const group = await FacilityGroup.findByIdAndDelete(req.params.id);
    if (!group)
        return res.status(404).json({ error: 'FacilityGroup not found' });
    res.status(204).send();
});
//# sourceMappingURL=facilityGroupController.js.map